/*
	1.csr방식에 대해서 설명하시오
	2.react프로젝트에서 public,src 폴더를 통해서 어떻게 어떤식으로 빌드가 되면서 화면 렌더링
	3.ssr방식에 비해 csr방식에 장점과 단점에 대해서 설명
*/
...
/*
	...
	(csr방식)
	- 사용자가 브라우저에 url입력해서 페이지요청하면 빈 html파일만 서버쪽에서 가져오고 그와동시에 jsx를 반환하는 리액트 컴포넌트 파일을 같이 불러옴
	- 리액트 컴포넌트가 모두 동작되면 빈 html문서에 동적으로 모든 컴포넌트가 렌더링됨
	- 초기 모든 서브페이지에 대한 파일들을 모두가져와서 url요청에 따라서 미리 가져온 리액트 컴포넌트를 바꾸면서 화면변경

	2.ssr방식에 비해 csr방식에 장점과 단점에 대해서 설명
	- 초기로딩속도가 ssr방식에 비해서는 오래 걸림
	- 처음에 빈 html파일을 가져오고 모든 리액트 컴포넌트가 마운트되기 전까지 사용자는 빈화면을 봐야 함 --> 검색엔진에 안좋음
	- 해결방법 (Next.js라는 framework을 이용해서 srr,csr방식이 결합된 hydration을 활용)
	- 해결방식 index.html을 불러오고 동적인 react컴포넌트 마운트되기 전까지 static 데이터를 미리 출력해서 검색엔진 최적화
	...
	- 기존에는 서버사이드 데이터로 전역state에 static하게 저장을 하다보니 실시간으로 자주바뀌는 데이터 경우에는
	...
	- 기존리액트 기능인 useContext를 활용한 커스텀훅을 활용
